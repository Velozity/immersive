...
/** Low  */
class Low extends Chariot.Command {
  constructor() {
    super();
    ...
    this.aliases = ['l']
    this.help = {
      message: 'Bet on low for your high low game',
      usage: 'low',
      example: 'low',
      visible: false
    };
  ...
  async execute(message, args, chariot) {
    ...
    if(Number(game[4]) > randomNumber) {
      ...
      const msg = await sendEmbed(message.channel,
        msgtypes.success,
        `High Low - Win`,
        `<@${message.author.id}> picked the right outcome **low**, you have won ${currencyInfo.currency_symbol}**${Number(game[2]) * 2} ${currencyInfo.currency}**!
...
Your number: **${randomNumber}**`,
      ...
      const msg = await sendEmbed(message.channel,
        msgtypes.general,
        `High Low - Lost`,
        `<@${message.author.id}> picked the wrong outcome, it was **high** :(
